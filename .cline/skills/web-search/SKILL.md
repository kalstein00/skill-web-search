...
2. 사전 설치된 Python과 uv를 사용한다. 다음 명령의 경로·검색어를 현재 셸에 맞게 인용한다. `<installed-python>`에는 사전 설치된 Python 실행파일 경로를 넣는다. uv가 PATH에 없지만 설치된 경로를 알고 있으면 `uv` 대신 그 실행파일의 절대 경로를 사용한다. Windows PowerShell에서는 실행파일 경로를 인용했으면 앞에 `&`를 붙인다.

   ```text
   uv run --offline --no-project --no-sync --no-python-downloads --no-managed-python --python "<installed-python>" "<skill-directory>/scripts/web_relay_client.py" --project-root "<project-root>" search "<query>"
   ```

   Python·uv가 없으면 준비 필요를 알리고 종료한다. 실행 도중 패키지 설치나 프로젝트 동기화를 수행하지 않는다. `--offline`은 uv의 다운로드를 막으며 LAN 서버 요청은 허용한다.
